...
class SummaryService(bentoml.BentoService):
    ...
    def dialog(self, input):
        tokenizer = self.artifacts.dialoggpt.get("tokenizer")
        model = self.artifacts.dialoggpt.get("model")
        ...
        # past_user_inputs = input["past_user_inputs"]
        # generated_responses = input["generated_responses"]
        response = ask(user_input,model,tokenizer)
        return response
